    ...
    * `true`
    ...
    if ACTIVE_MODEL_TYPE == "pytorch":
        ...
        if pt_model is None:
            ...
            return
        st.write(
            f"**Active Model:** {type(pt_model).__name__} (Deep Learning)")

    elif ACTIVE_MODEL_TYPE == "sklearn":
        sk_vectorizer, sk_classifier = load_sklearn_pipeline(
            "word2vec_lg_pipeline.pkl")
        if sk_classifier is None:
            st.error("Scikit-Learn file not found in the `models/` directory.")
            return
        st.write(
            f"**Active Model:** {type(sk_classifier).__name__} (Classical ML)")
